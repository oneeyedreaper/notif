...
import { Mail, MessageSquare, Bell, Clock, Loader2, Save, Check, Phone, CheckCircle, AlertCircle, Shield } from 'lucide-react'
...
export default function SettingsPage() {
  ...
  const [phoneVerifyMessage, setPhoneVerifyMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null)
  ...
  const handleSendPhoneCode = async () => {
    ...
    try {
      ...
      setPhoneVerifyMessage({ type: 'error', text: error.message || 'Failed to send code' })
    } finally {
  ...
  const handleVerifyPhone = async () => {
    if (phoneCode.length !== 6) {
      setPhoneVerifyMessage({ type: 'error', text: 'Please enter a 6-digit code' })
      return
    }
    ...
    try {
      ...
      setPhoneVerifyMessage({ type: 'error', text: error.message || 'Verification failed' })
    } finally {
      setPhoneVerifyLoading(false)
    }
  }
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                ...
                {verificationStatus.emailVerified ? (
                  ...
                  <Button size="sm" variant="outline" onClick={handleResendEmailVerification}>
                    Resend Verification
                  </Button>
                )}
              </div>
              ...
              <div className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                ...
                {verificationStatus.phoneVerified ? (
                  ...
                  <Button size="sm" variant="outline" onClick={() => setShowPhoneVerify(true)}>
                    Verify Phone
                  </Button>
                ) : (
                ...
                <div className="p-4 border rounded-lg space-y-4">
                  ...
                  {!codeSent ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex gap-2">
                        <Button onClick={handleVerifyPhone} disabled={phoneVerifyLoading || phoneCode.length !== 6}>
                          {phoneVerifyLoading ? (
                            <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                          ) : null}
                          Verify
                        </Button>
                        <Button variant="outline" onClick={handleSendPhoneCode} disabled={phoneVerifyLoading}>
                          Resend Code
                        </Button>
                        <Button variant="ghost" onClick={() => { setShowPhoneVerify(false); setCodeSent(false); setPhoneCode(''); }}>
                          Cancel
                        </Button>
                      </div>
                    </div>
                  ...
                  {phoneVerifyMessage && (
                    <div className={`flex items-center gap-2 text-sm ${
                      phoneVerifyMessage.type === 'success' ? 'text-green-500' : 'text-destructive'
                    }`}>
              ...
              <CardContent>
                <div className="flex gap-2">
                  {(['instant', 'daily', 'weekly'] as const).map((freq) => (
                    <Button
                      key={freq}
                      variant={preferences.emailFrequency === freq ? 'default' : 'outline'}
            ...
            <CardContent>
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  <Label htmlFor="quietStart" className="text-sm mb-2 block">Start</Label>
                  <Input
                    id="quietStart"
                    type="time"
                ...
                <div className="flex-1">
                  <Label htmlFor="quietEnd" className="text-sm mb-2 block">End</Label>
                  <Input
                    id="quietEnd"
                    type="time"
